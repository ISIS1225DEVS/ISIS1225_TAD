...
def insertVertex ( graph, vertex ):
    ...
    """ 
    edges = lt.newList()
    infoVertex = {'edges':edges, 'marked':False, 'edgeTo':None, 'distTo':-1}
    map.put (graph['vertices'], vertex, infoVertex)
    return graph
...
def edges (graph):
    ...
    """ 
    lstvalue = map.valueSet(graph['vertices'])
    lstresp = lt.newList ()
    itervertex = it.newIterator (lstvalue)
    while it.hasNext (itervertex):
        ver_value = it.next (itervertex)
        iteredge = it.newIterator (ver_value['edges'])
...
def addEdge (graph, vertexa, vertexb, weight):
    ...
    """ 
    # Se crea el arco
    edgeA =  e.newEdge (vertexa,vertexb, weight)
    ...
    lt.addLast (entrya['value']['edges'], edgeA)
    lt.addLast (entryb['value']['edges'], edgeB)
